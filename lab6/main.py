...
def start_command(message):
    ...
    bot.send_message(
        user_id,
        "Это бот для работы с заметками. Выберите действие:",
...
def process_callback(call: CallbackQuery):
    ...
    if call.data == "add_note":
        user_states[user_id] = STATES["ADDING_NOTE"]
        ...
        user_states[user_id] = STATES["VIEWING_NOTES"]
...
def add_note_handler(message):
    ...
    if user_states.get(user_id) == STATES["ADDING_NOTE"]:
...
def delete_note_handler(call: CallbackQuery):
    user_id = call.from_user.id
    if user_states.get(user_id) == STATES["VIEWING_NOTES"]:
        if user_id in notes and notes[user_id]:
            keyboard = InlineKeyboardMarkup(row_width=1)
